<template>
  ...
  <v-container v-else-if="report">
    <v-card>
      <v-card-title class="text-h5">报告审核详情 - {{ report.id }}</v-card-title>
      <v-card-text>
        <v-row>
          <v-col cols="12" md="6">
            <v-text-field
...
import { getReportEntry, submitReportEntry, withdrawReportEntry } from '@/services/report.service';
...
async function submitReview() {
  ...
  try {
    ...
    await submitReportEntry(report.value.id);
    report.value.status = '已审核';
    alert('报告提交审核成功');
  } catch (err) {
    alert('报告提交审核失败: ' + (err instanceof Error ? err.message : '未知错误'));
    console.error(err);
  } finally {
...
async function withdrawReview() {
  ...
  try {
    ...
    await withdrawReportEntry(report.value.id);
    report.value.status = '已提交';
    alert('报告撤回审核成功');
  } catch (err) {
    alert('报告撤回审核失败: ' + (err instanceof Error ? err.message : '未知错误'));
    console.error(err);
  } finally {
...
function goBack() {
  router.push({ name: 'ReportReviewList' });
}
</script>
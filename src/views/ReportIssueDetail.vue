<template>
  ...
  <v-container v-else-if="report">
    <v-card>
      <v-card-title class="text-h5">报告发放详情 - {{ report.id }}</v-card-title>
      <v-card-text>
        <v-row>
          <v-col cols="12" md="6">
            <v-text-field
...
import { getReportEntry, submitIssue, withdrawIssue } from '@/services/report.service';
...
async function handleSubmitIssue() {
  ...
  try {
    ...
    await submitIssue(report.value.id);
    report.value.status = '已发放';
    alert('报告提交发放成功');
  } catch (err) {
    alert('报告提交发放失败: ' + (err instanceof Error ? err.message : '未知错误'));
    console.error(err);
  } finally {
...
async function handleWithdrawIssue() {
  if (!report.value) return;
  
  try {
    withdrawing.value = true;
    await withdrawIssue(report.value.id);
    report.value.status = '已批准';
    alert('报告撤回发放成功');
  } catch (err) {
    alert('报告撤回发放失败: ' + (err instanceof Error ? err.message : '未知错误'));
    console.error(err);
  } finally {
...
function goBack() {
  router.push({ name: 'ReportIssueList' });
}
</script>